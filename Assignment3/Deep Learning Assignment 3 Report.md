...
![png](https://snowcharm.oss-cn-shenzhen.aliyuncs.com/markdown202305171531188.png)

**Halfway through training: (epoch 40)**

![png](https://snowcharm.oss-cn-shenzhen.aliyuncs.com/markdown202305171531552.png)

**Halfway through training: (epoch 100)**

![png](https://snowcharm.oss-cn-shenzhen.aliyuncs.com/markdown202305171531597.png)

**After the training has terminated: (epoch 500)**

![png](https://snowcharm.oss-cn-shenzhen.aliyuncs.com/markdown202305171531510.png)

From the figures above, we can find that the figures we generated are better and better when the number of epochs increases. At the start of the training, the generated images are ambiguous. When the epoch is 50, the model can generate rough outlines of handwritten digits that are recognizable to the naked eye as digits, but it does not excel in generating details. When the epoch is 100, the model excels in generating details of the digits, but some parts of the generated images still appear blurry and unclear. When the training has terminated, the generated images by the model are almost indistinguishable from the original dataset, indicating that the image quality generated by the model is already quite high.

After training, we load the model state saved in training. We sample 2 images from a zero-tensor and a one-tensor, and we use 7 interpolation steps to result in 9 images (including start and end point).
...
From the result we can see that the generated image shows that the entire picture presents a gradual transition effect from the first to the last one.
